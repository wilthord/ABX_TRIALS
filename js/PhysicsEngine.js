...
PhysicsEngineClass.prototype.addBody = function (entityDef) {
    ...
    if(entityDef.filterMaskBits) fixtureDefinition.filter.maskBits = entityDef.filterMaskBits;

    if(entityDef.filterCategoryBits) fixtureDefinition.filter.categoryBits = entityDef.filterCategoryBits;

    if(entityDef.forma){
        if(entityDef.forma === 'circulo'){
            fixtureDefinition.shape = new CircleShape();
            fixtureDefinition.shape.SetRadius(entityDef.radio);
        }else{
    ...
    if(entityDef.userData){
        body.SetUserData(entityDef.userData);
    }
